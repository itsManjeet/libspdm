...
{
    libspdm_ec_context *sm2_ctx;

    if (sm2_context == NULL) {
        return;
    }

    sm2_ctx = (libspdm_ec_context *)sm2_context;
                                 ...
                                 size_t public_key_size)
{
    ...
    pkey = EC_CTX_TO_EVP_PKEY(sm2_context);
                                  ...
                                  size_t *public_size)
{
    ...
    *public_size = pub_key_len - 1;
    if (public_data != NULL) {
        /* pub_key_buffer[0] = 0x04 indicates the uncompressed format.
         * Only the raw key bytes (excluding this prefix) should be copied. */
