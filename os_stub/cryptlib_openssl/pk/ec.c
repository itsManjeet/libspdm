                            ...
                            size_t *public_key_size)
{
    ...
    evp_pkey = (EVP_PKEY *)ec_context;
    if (EVP_PKEY_get_octet_string_param(evp_pkey, OSSL_PKEY_PARAM_PUB_KEY, key, sizeof(key), &len) <= 0) {
        return false;
    }
    ...
    *public_key_size = len - 1;
    memcpy(public_key, key+1, *public_key_size);
                             ...
                             size_t *public_size)
{
    ...
    uint8_t comp_public_key[66 * 2 + 1];
    size_t comp_public_key_size = 0;
    bool result = false;
    ...
    if (EVP_PKEY_get_octet_string_param(gen_pkey, OSSL_PKEY_PARAM_PUB_KEY, comp_public_key, sizeof(comp_public_key), &comp_public_key_size) <= 0) {
        goto cleanup_gen_pkey;
    }

    if (*public_size < comp_public_key_size - 1) {
        *public_size = comp_public_key_size - 1;
        goto cleanup_gen_pkey;
    }

    *public_size = comp_public_key_size - 1;
    memcpy(public_data, comp_public_key+1, *public_size);

    if (EVP_PKEY_set1_EC_KEY(evp_pkey, (EC_KEY *) EVP_PKEY_get0_EC_KEY(gen_pkey)) <= 0) {
        goto cleanup_gen_pkey;
    }
    result = true;
