...
void *libspdm_mlkem_new_by_name(size_t nid)
{
    ...
    /* Return double pointer for compatibility with DH memory management */
    EVP_PKEY **kem_context = malloc(sizeof(EVP_PKEY *));
    if (kem_context == NULL) {
        EVP_PKEY_free(pkey);
        return NULL;
    }
...
{
    if (kem_context != NULL) {
        EVP_PKEY **pkey_ptr = (EVP_PKEY **)kem_context;
        if (*pkey_ptr != NULL) {
...
{
    EVP_PKEY **ctx_ptr = (EVP_PKEY **)kem_context;
    EVP_PKEY *pkey = *ctx_ptr;
    int ret;
 ...
 *
 * NOTE: This function assumes kem_context is actually an EVP_PKEY** (double pointer)
 * for ML-KEM contexts created by libspdm_mlkem_new_by_nid(), which returns
 * a double pointer to allow key replacement.
 *
 * @param[in, out]  kem_context  Pointer to KEM context being set (EVP_PKEY** for ML-KEM).
...
bool libspdm_mlkem_set_privkey(void *kem_context, const uint8_t *key_data, size_t key_size)
{
    ...
    /* For ML-KEM, kem_context is actually EVP_PKEY** (double pointer) */
    evp_key_ptr = (EVP_PKEY **)kem_context;
    old_pkey = *evp_key_ptr;
